...
class PredictionResult:
    """Complete prediction result for a date/location."""
    prediction_date: str
    overall_score: int
    score_label: str
    confidence: str
...
class PredictionEngine:
    ...
    def get_migration_season(self, forecast_date: date, latitude: float) -> tuple[str, str]:
        """
        Determine migration season and type based on date and location.

        Returns: (season, migration_type)
        """
        month = forecast_date.month

        if month in [3, 4, 5]:  # March-May
            return ("spring", "neotropical")
        elif month in [8, 9, 10, 11]:  # August-November
    ...
    def is_coastal(self, latitude: float, longitude: float) -> bool:
        """Check if location is in a coastal area (rough approximation)."""
        # Gulf Coast
    ...
    def calculate_front_score(
        self,
        pressure_delta_3h: Optional[float],
        pressure_delta_24h: Optional[float],
        temperature_delta_24h: Optional[float],
        ...
        Cold fronts are the primary driver of fallout events.
        """
        score = 0
        descriptions = []

        # Direct frontal passage indicator
        if is_frontal_passage:
            score += 10
            descriptions.append("Frontal passage detected")

        # Rapid pressure drop indicates front passage
        ...
        if temperature_delta_24h is not None:
            if season == "spring":
                # Spring: cold front bringing cooler air stops northward migration
                ...
                # Fall: cold front pushes birds south
    ...
    def calculate_wind_score(
        ...
        Headwinds force birds to land; strong winds increase fallout likelihood.
        """
        score = 0
        descriptions = []
        ...
        # Determine if wind is a headwind based on season
        is_headwind = False
        if season == "spring":
            # North winds oppose northward migration (315-360, 0-45)
            if wind_direction >= 315 or wind_direction <= 45:
                is_headwind = True
                descriptions.append("Northerly headwinds")
        elif season == "fall":
            # South winds oppose southward migration (135-225)
            if 135 <= wind_direction <= 225:
                is_headwind = True
                descriptions.append("Southerly headwinds")

        # Score based on headwind strength
        if is_headwind:
            if wind_speed >= 40:
                score += 15
        ...
        # Strong gusts increase fallout likelihood
        ...
        # Pacific coast: easterly winds are unusual and significant
    ...
    def calculate_precipitation_score(
        ...
        precip_amount: Optional[float],
        is_coastal: bool
    ) -> ScoreComponent:
        """
        Calculate score for precipitation (0-20 points).

        Rain forces birds to land and can disorient nocturnal migrants.
        """
        score = 0
        descriptions = []
        ...
        # Over-water precipitation especially impactful for Gulf Coast trans-Gulf migrants
    ...
    def calculate_pressure_score(
        self,
        pressure_current: Optional[float],
        pressure_delta_24h: Optional[float],
    ...
    def calculate_visibility_score(
        ...
        Poor visibility and low clouds disorient nocturnal migrants.
        """
        score = 0
        descriptions = []
    ...
    def calculate_temperature_score(
        ...
        if season == "spring":
            # Cold snap during spring migration
            if temperature < 0:
                score += 5
            ...
            # Extreme cold drives irruptions
            if temperature < -20:
                score += 5
        ...
        if apparent_temperature is not None and temperature is not None:
            if apparent_temperature < temperature - 10:
                score += 2
                descriptions.append(f"Significant wind chill")
    ...
    def generate_summary(
        ...
        top_factors = [name for score, name in factor_scores if score >= 5][:2]
        if top_factors:
    ...
    def generate_predictions(
        ...
        Groups hourly forecasts by day and returns daily predictions.
        """
        ...
        for date_str, day_forecasts in sorted(daily_forecasts.items()):
            ...
            # Use peak conditions from the day (typically afternoon/evening)
            # Find the forecast with the most significant weather
            best_forecast = max(
                day_forecasts,
                key=lambda f: (
